	...
	<head><script type="text/javascript" src="/StudentInfo/utils/scripts/flat-ui.js"></script><meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no"><link rel="shortcut icon" href="/StudentInfo/utils/image/favicon.ico" type="image/x-icon" />
		...
		$(document).ready(
			function() {
				$("#noticeid").change(
						function() {
							$.post("/StudentInfo/AjaxHandler/existNoticeid?Noticeid="
									+ $("#noticeid").val(), function(data, status) {
								$("#noticeid").html(data);
							})
	...
	<div class="container">
		<div class="row">
			<div class="col-md-6">
				...
				<form action="/StudentInfo/AdminHandler/addNotice" method="post">
					<h6>公告编号</h6>
					
						<input type="text" name="noticeid" id="noticeid" class="form-control" oninput = "value=value.replace(/[^\d]/g,'')" maxlength="5"></input> 
						<span id="counttid" style="color: #ff0000;"></span>
					   <h6>公告内容</h6>
						
						<textarea name="noticecontent" cols="20" rows="6" class="form-control"></textarea>
						<h6>公告时间</h6>
						<input type="text" name="noticetime" class="form-control" onkeyup="this.value=this.value.replace(/(^\s+)|(\s+$)/g,'');" maxlength="16"></input>
